...
function boeResize()
{
     ...
     $.boeSettings.screenHeight=window.screen.height;
         ...
         $.boeSettings.windowWidth=$(window).width();
	 	 boeConfig();
	 	 if($.boeSettings.androidRotation  && $.boeSettings.desktop)
	 	 {
	 	 	setTimeout(function(){
	 	 	$.boeSettings.androidRotation=false;
	 	    $.boeSettings.bannerHome.css("background-image","url('')"); 
     	    $.boeSettings.bannerHome.css("background","url('../img/treesbgA.jpg')");
     	    },1000);
     	}
}
...
/*based on screen width,choose video files */
function setVideoFileGroup()
{   
	var curString="S.",
	       newString=".",
	       vidPropArray = [settings.com.vidsrc,settings.des.vidsrc,settings.his.vidsrc],
    ...
    { 
     ...
     for (i=0;i<limit;i++)
     	      {   
     	      	  ...
     	      	  {      
     	      	        this.src=this.src.replace(curString,newString);
     	      	  });  
     	      }
}
...
{     makeIconText($button);
	  ...
	  shiftSpace = vidOffset-$button.width();
	  leftShift = (shiftSpace > 10) ? shiftSpace/2:5;
