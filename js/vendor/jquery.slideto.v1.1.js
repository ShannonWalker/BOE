...
(function($) {
	$.fn.slideto = function(options) {
		...
		$(elem).each(function() {
			...
			console.log("anchor is " + anchor.attr('id'));
			console.log("anchor top" + anchor.offset().top);
			
			topTarget = options.topAdjust ? options.topAdjust : 0;
			topTarget += anchor.offset().top;
			
			$(e).on('click', function(){
				if (o.triggerAnchor)
				{
			   anchor.click();console.log("top target " +topTarget);
			   }
			   else {console.log("no trigger on anchor");}
			    $('html, body').animate({
					scrollTop: topTarget,
